import { useState, useMemo, useCallback, TouchEvent, RefObject, ReactNode } from 'react'
...
const usePageSwiper = (props: Props) => {
  const [state, updateState] = useState<State>(merge(defaultState(), { current: props.current }))
  ...
  const handleStartSwipe = useCallback((event: TouchEvent<HTMLElement>) => {
    event.persist()
    updateState((_state) => {
  ...
  const handleMoveSwipe = useCallback((event: TouchEvent<HTMLElement>) => {
    event.persist()
    updateState((_state) => {
      if (!_state.isTouchDown || _state.isProcessTransit) return _state
  ...
  const handleEndSwipe = useCallback(
    (event: TouchEvent<HTMLElement>) => {
      event.persist()
      if (props.ref.current === null || state.isProcessTransit) return
      const isPrev = state.offsetX >= 0
      const shouldUpdatePage = Math.abs(state.offsetX) >= options.threshold
      const { width } = props.ref.current.getBoundingClientRect()
